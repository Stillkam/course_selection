...
$conn = new mysqli('localhost','root' , '941126xiangjie', 'courses');
    ...
    $sql = "update sc set sc.Grade='{$_POST['grade']}' where sc.SID={$_POST['SID']} and sc.CID='{$_POST['CID']}'";
    if ($conn->query($sql) == TRUE) {
        echo '<script>alert("Mark successfully!")</script>';
    }
    else
        echo '<script>alert("Mark Failed!")</script>';
}
?>
    <!-- navbar -->
...
$conn = new mysqli('localhost','root' , '941126xiangjie', 'courses');
    ...
    while($row1 = $result1->fetch_assoc()){
        ...
        while($row2 = $result2->fetch_assoc()){
            ...
            echo '<form method="post" action="mark.php"><td><input type="text" name="grade" class="span8 inline-input"  style="width: 80px;">
            </td><td>
            <input  name="SID" value="'.$row2['SID'].'" type="hidden">
            <input  name="CID" value="'.$row1['CID'].'" type="hidden">
            <input type="submit" value="Mark" class="btn-flat danger"/>
            </td><tr></form>';
        }
        echo '</table>';
        }
