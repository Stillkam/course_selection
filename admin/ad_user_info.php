    ...
    <div class="content">
        ...
        <thead>
             <tr>
                          <th>ID</th>
                          <th>User Type</th>
                          <th><a class="btn-flat" href="ad_user_add.php">&nbsp&nbspADD&nbsp&nbsp</a></th>
                          <th></th>
             </tr>
        </thead>
            <tbody>
...
$conn = new mysqli('localhost','root' , '941126xiangjie', 'courses');
    ...
    $sql = "select ID,UserTP from user ";
$result=$conn->query($sql); 
while($row = $result->fetch_assoc()){
        echo '<tr><td>'.$row['ID'].'</td>';
        echo '<td>'.$row['UserTP'].'</td>';
        echo '<td style="width: 100px;"><form method="post" action="ad_user_mf.php">
        <input  name="ID" value="'.$row['ID'].'" type="hidden">
        <input type="submit" value="modify" class="btn-flat white"> </form></td>
        <td><form method="post" action="ad_user_de.php">
        <input  name="ID" value="'.$row['ID'].'" type="hidden">
