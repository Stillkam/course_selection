...
<html>
<head>
	<title>Student INFO Query</title>
...
$std=$_POST['Std'];
 $conn = new mysqli('localhost','root' , '941126xiangjie', 'courses');
    ...
    $sql = "select SID,SNAME,college from student where student.SID like '%{$std}%' or student.SNAME like '%{$std}%'";
...
while($row = $result->fetch_assoc()){
     echo '<table class="table table-hover" >
        ...
        $sql1="select CID,Grade from sc where sc.SID={$row['SID']} ";
        $result1 = $conn->query($sql1);
        while($row1 = $result1->fetch_assoc()){
            $sql2="select Cname from course where course.CID='{$row1['CID']}'";
