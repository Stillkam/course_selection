    ...
    <div class="content">
        ...
        <thead>
             <tr>
                          ...
                          <th>Teacher Name</th>
                          <th>College</th>
                          <th><a class="btn-flat" href="ad_t_add.php">&nbsp&nbspADD&nbsp&nbsp</a></th>
                          <th></th>
             </tr>
        </thead>
            <tbody>
...
$conn = new mysqli('localhost','root' , '941126xiangjie', 'courses');
    ...
    $sql = "select TID,Tname,college from teacher ";
$result=$conn->query($sql); 
while($row = $result->fetch_assoc()){
        echo '<tr><td>'.$row['TID'].'</td>';
        echo '<td>'.$row['Tname'].'</td>';
        echo '<td>'.$row['college'].'</td>';
        echo '<td><form method="post" action="ad_t_mf.php">
        <input  name="TID" value="'.$row['TID'].'" type="hidden">
        <input type="submit" value="modify" class="btn-flat white"> </form></td>
        <td><form method="post" action="ad_t_de.php">
        <input  name="TID" value="'.$row['TID'].'" type="hidden">
