...
$sql1="select * from sc where  CID='{$courseID}' and sc.SID={$_SESSION['user']}";
$result1= $conn->query($sql1); 
if($result1->num_rows>0){ 
    $sql="delete from sc where sc.SID={$_SESSION['user']} and sc.CID='{$courseID}'";
    if($conn->query($sql)==true)
        echo '<script>alert("Course Dropped Successfully! ")</script>';
}
else
    echo '<script>alert("CID Is Wrong!")</script>';
?>
	<!-- main container -->
    <div class="content">
